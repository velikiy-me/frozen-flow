...
object CompositeBodyTemplateProcessor : ResponseBodyTemplateProcessor {

    override fun process(body: ResponseBodyAccessor, bodyTemplate: Response.Template) {
        when (bodyTemplate.type) {
            "value" -> ValueBodyTemplateProcessor.process(body, bodyTemplate)
            else -> error("Unknown template type: '${bodyTemplate.type}'!")
        }
    }
}
