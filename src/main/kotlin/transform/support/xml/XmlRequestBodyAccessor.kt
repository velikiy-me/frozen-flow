...
class XmlRequestBodyAccessor(private val stub: StubMapping) : RequestBodyAccessor {

    private val pattern = stub.request.bodyPatterns[0] as EqualToXmlPattern
    private val content = Xml.parse(pattern.value)
    ...
    override fun close() {
        if (changed) {
            stub.request.bodyPatterns[0] = EqualToXmlPattern(content.toString())
            changed = false
        }
    }
}
