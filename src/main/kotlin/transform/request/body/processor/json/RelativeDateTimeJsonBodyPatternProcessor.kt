...
import me.velikiy.frozenflow.transform.request.body.processor.common.*
import me.velikiy.frozenflow.transform.support.*

object RelativeDateTimeJsonBodyPatternProcessor : RequestBodyPatternProcessor {
    ...
    override fun process(
        ...
        val patternBuilder = RelativeDateTimePatternBuilder(requestBodyPattern, metadata)

        body.map(requestBodyPattern.ref) { currentValue, ref, _ ->
            request.bodyPatterns.add(
                MatchesJsonPathPattern(
                    ref,
                    patternBuilder.build(currentValue, ref)
                )
            )
