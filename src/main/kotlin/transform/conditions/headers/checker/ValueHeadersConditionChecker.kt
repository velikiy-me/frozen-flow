...
object ValueHeadersConditionChecker : RequestHeadersConditionChecker {

    override fun check(headers: Map<String, MultiValuePattern>, condition: Condition): Boolean {
        requireNotNull(condition.value) {
            "Value must be specified in headers value condition for reference: '${condition.ref}'!"
        }
        val pattern = headers[condition.ref]
        return pattern?.expected == condition.value
    }
}
